...
if (ini_set('magic_quotes_runtime', 0) /*get_magic_quotes_runtime()*/) {
...
class Xcrud
{
    ...
    protected function render_custom_csv()
    {
        ...
        $table_name = $this->_clean_file_name(trim(html_entity_decode(String1::toString($this->table_name), ENT_QUOTES, 'utf-8')));
    ...
    protected function _update($postdata, $primary)
    {
        ...
        foreach ($postdata as $key => $val) {
            if (isset($fields[$key]) && !isset($this->locked_fields[$key]) && !isset($this->custom_fields[$key])) {
                ...
                elseif (isset($this->point_field[$key]) && trim(String1::toString($val)))
                {
                ...
                isset($this->pass_var['edit'][$key])) ? $db->escape($val, true) : $db->escape(trim(String1::toString($val)), false, $this->field_type[$key],
                $this->field_null[$key], isset($this->bit_field[$key])));
                */
    ...
    protected function _cut($string, $field, $wordsafe = true, $dots = true)
    {
        ...
        $string = html_entity_decode(String1::toString($string), ENT_QUOTES, Xcrud_config::$mbencoding);
    ...
    protected function make_links($txt)
    {
        if ($this->links_label)
            ...
            return preg_replace('/(http:\/\/|https:\/\/)([^\s]+)/u', '<a target="_blank" href="$1$2">$1$2</a>', String1::toString($txt));
    }
    ...
    public static function check_url($url, $scr_url = false)
    {
         ...
         */
        $xcrud_ehex_path = PATH_LIB_ASSETS . 'library/xcrud';
        ...
        $scheme = $scheme === "http://" && $isForceHTTPS()?  "https://" : "http://";
