...
class Number1
{
     ...
     * @return float|int (return only percentage B of As. e.g percetage 5 of 40 = 12.5%)
     */
     ...
     * @return float|int (return only percentage of the value passed in)
     */
    ...
    {
        $ans = $numbers[0];
        for ($i = 1; $i < count($numbers); $i++) $ans = ((($numbers[$i] * $ans)) / (self::getGCD($numbers[$i], $ans)));
        return $ans;
    }
    ...
    {
        if (count($numbers) > 2) {
            return array_reduce($numbers, [self::class, 'getGCD']);
        }
        $r = $numbers[0] % $numbers[1];
        return $r === 0 ? abs($numbers[1]) : self::getGCD($numbers[1], $r);
    }
    ...
    public static function getSurroundingValues($totalValueCount, $currentIndex = 1, $maxPageCount = 6)
    {
        ...
        $maxPageCount = $total > 10 && $maxPageCount > $total  ? $total / 2 : $maxPageCount;
