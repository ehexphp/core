...
//        return (is_array($urlRaw) ? $urlRaw[0] . "//" : "") . $data[0];
     ...
     * @param callable|null $elseCallback
     * @return mixed
     */
    ...
    static function cURL_lite($url, array $header = ['Accept: application/json']): ResultObject1
    {
        ...
        if ($reply === false) return ResultStatus1::falseMessage(curl_error($crl));
        curl_close($crl);
        return ResultObject1::make(json_decode($reply, true));
    }
}
